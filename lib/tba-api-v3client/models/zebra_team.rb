  ...
  class ZebraTeam
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TBA API v3 Ruby Client::ZebraTeam` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TBA API v3 Ruby Client::ZebraTeam`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'xs')
        if (value = attributes[:'xs']).is_a?(Array)
          self.xs = value
        end
      end

      if attributes.key?(:'ys')
        if (value = attributes[:'ys']).is_a?(Array)
          self.ys = value
        end
      end
    end
    ...
    def list_invalid_properties
      ...
      if @xs.nil?
        invalid_properties.push('invalid value for "xs", xs cannot be nil.')
      end

      if @ys.nil?
        invalid_properties.push('invalid value for "ys", ys cannot be nil.')
      end
