  ...
  class EventRankingRankings
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TBA API v3 Ruby Client::EventRankingRankings` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TBA API v3 Ruby Client::EventRankingRankings`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'extra_stats')
        if (value = attributes[:'extra_stats']).is_a?(Array)
          self.extra_stats = value
        end
      end
    ...
    def list_invalid_properties
      ...
      if @dq.nil?
        invalid_properties.push('invalid value for "dq", dq cannot be nil.')
      end
    ...
    def hash
      [matches_played, qual_average, extra_stats, sort_orders, record, rank, dq, team_key].hash
    end
