  ...
  class Match
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TBA API v3 Ruby Client::Match` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TBA API v3 Ruby Client::Match`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'comp_level')
        self.comp_level = attributes[:'comp_level']
      end

      if attributes.key?(:'set_number')
        self.set_number = attributes[:'set_number']
      end

      if attributes.key?(:'match_number')
      ...
      if attributes.key?(:'winning_alliance')
      ...
      if attributes.key?(:'predicted_time')
        self.predicted_time = attributes[:'predicted_time']
      end

      if attributes.key?(:'post_result_time')
